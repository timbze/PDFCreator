        ...
        protected override void NotifyUserAboutFailedJob()
        {
            ...
            switch (Job.ErrorType)
            {
                case JobError.Ghostscript:
                    errorText = _translator.GetTranslation("InteractiveWorkflow", "GhostscriptError", "Internal Ghostscript error");
                    break;
                default:
                    errorText = _translator.GetTranslation("InteractiveWorkflow", "UnknownError", "Unkown internal error");
                    break;
            }

            var caption = _translator.GetTranslation("InteractiveWorkflow", "Error", "Error");
            var opener = _translator.GetFormattedTranslation("InteractiveWorkflow", "ErrorWhileConverting", "PDFCreator was not able to convert the document, because an error occured:\r\n{0}\r\n\r\nYou can find additional information in the log file.", errorText);

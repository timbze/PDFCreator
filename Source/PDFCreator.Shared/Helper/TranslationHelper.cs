...
namespace pdfforge.PDFCreator.Shared.Helper
{
    ...
    public class TranslationHelper
    {
        ...
        public string TranslationPath
        {
            get
            {
                return LanguageLoader.TranslationFolder ?? "";
            }
        }

        public bool IsInitialized
        {
            get { return _translator != null; }
        }
        ...
        {
            string appDir = _assemblyHelper.GetCurrentAssemblyDirectory();

            var translationPathCandidates = new[] { Path.Combine(appDir, @"Languages"), Path.Combine(appDir, @"..\..\Languages") };
        ...
        public void TranslateProfileList(IList<ConversionProfile> profiles)
        {
            foreach (ConversionProfile p in profiles)
            {
                try
                {
                    string translation = _translator.GetTranslation("ProfileNameByGuid", p.Guid);

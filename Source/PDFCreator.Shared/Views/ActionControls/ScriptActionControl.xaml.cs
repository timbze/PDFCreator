...
namespace pdfforge.PDFCreator.Shared.Views.ActionControls
{
    public partial class ScriptActionControl : ActionControl
    {
        ...
        public ScriptActionControl()
        {
            ...
            DisplayName = TranslationHelper.Instance.TranslatorInstance.GetTranslation("ScriptActionSettings", "DisplayName", "Run script");
            Description = TranslationHelper.Instance.TranslatorInstance.GetTranslation("ScriptActionSettings", "Description", "The script action runs a custom script or application after the conversion. This allows you to further process the output.");
        ...
        {
            var title = TranslationHelper.Instance.TranslatorInstance.GetTranslation("ScriptActionSettings", "SelectScriptTitle", "Select script file");
            var filter = TranslationHelper.Instance.TranslatorInstance.GetTranslation("ScriptActionSettings", "ExecutableFiles", "Executable files")
                + @" (*.exe, *.bat, *.cmd)|*.exe;*.bat;*.cmd|"
                + TranslationHelper.Instance.TranslatorInstance.GetTranslation("ScriptActionSettings", "AllFiles", "All files")
                + @"(*.*)|*.*";

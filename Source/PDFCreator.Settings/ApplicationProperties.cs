...
namespace pdfforge.PDFCreator.Core.Settings
{
	public class ApplicationProperties {
		...
		{
			try { NextUpdate = DateTime.Parse(data.GetValue(@"" + path + @"NextUpdate"), CultureInfo.InvariantCulture); } catch { NextUpdate = DateTime.Now;}
			try { SettingsVersion = int.Parse(data.GetValue(@"" + path + @"SettingsVersion"), CultureInfo.InvariantCulture); } catch { SettingsVersion = 2;}
		}

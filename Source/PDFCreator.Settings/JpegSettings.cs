...
namespace pdfforge.PDFCreator.Core.Settings
{
	...
	public class JpegSettings {
		...
		public void ReadValues(Data data, string path)
		{
			...
			try { Dpi = int.Parse(data.GetValue(@"" + path + @"Dpi"), CultureInfo.InvariantCulture); } catch { Dpi = 150;}
			try { Quality = int.Parse(data.GetValue(@"" + path + @"Quality"), CultureInfo.InvariantCulture); } catch { Quality = 75;}
		}

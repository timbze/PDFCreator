...
namespace pdfforge.PDFCreator.Core.Settings
{
	...
	public class CompressColorAndGray {
		...
		public void ReadValues(Data data, string path)
		{
			...
			try { Dpi = int.Parse(data.GetValue(@"" + path + @"Dpi"), CultureInfo.InvariantCulture); } catch { Dpi = 300;}
			try { Enabled = bool.Parse(data.GetValue(@"" + path + @"Enabled")); } catch { Enabled = true;}
			try { JpegCompressionFactor = double.Parse(data.GetValue(@"" + path + @"JpegCompressionFactor"), CultureInfo.InvariantCulture); } catch { JpegCompressionFactor = 0.66;}

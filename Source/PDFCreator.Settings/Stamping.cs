...
namespace pdfforge.PDFCreator.Core.Settings
{
	...
	public class Stamping {
		...
		public void ReadValues(Data data, string path)
		{
			...
			try { FontOutlineWidth = int.Parse(data.GetValue(@"" + path + @"FontOutlineWidth"), CultureInfo.InvariantCulture); } catch { FontOutlineWidth = 2;}
			try { FontSize = float.Parse(data.GetValue(@"" + path + @"FontSize"), CultureInfo.InvariantCulture); } catch { FontSize = 48;}

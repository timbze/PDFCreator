...
namespace pdfforge.PDFCreator.Core.Settings
{
	...
	public class Signature {
		...
		public void ReadValues(Data data, string path)
		{
			...
			try { LeftX = int.Parse(data.GetValue(@"" + path + @"LeftX"), CultureInfo.InvariantCulture); } catch { LeftX = 100;}
			try { LeftY = int.Parse(data.GetValue(@"" + path + @"LeftY"), CultureInfo.InvariantCulture); } catch { LeftY = 100;}
			try { RightX = int.Parse(data.GetValue(@"" + path + @"RightX"), CultureInfo.InvariantCulture); } catch { RightX = 200;}
			try { RightY = int.Parse(data.GetValue(@"" + path + @"RightY"), CultureInfo.InvariantCulture); } catch { RightY = 200;}
			...
			try { SignatureCustomPage = int.Parse(data.GetValue(@"" + path + @"SignatureCustomPage"), CultureInfo.InvariantCulture); } catch { SignatureCustomPage = 1;}
		...
		public void StoreValues(Data data, string path)
		{
			...
			data.SetValue(@"" + path + @"SignatureCustomPage", SignatureCustomPage.ToString(CultureInfo.InvariantCulture));
